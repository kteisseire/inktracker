...
const FROM_EMAIL = process.env.EMAIL_FROM || 'InkTracker <onboarding@resend.dev>';
...
export async function sendPasswordResetEmail(to: string, resetUrl: string) {
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: 'Réinitialisation de votre mot de passe InkTracker',
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 16px;">
        ...
        <p style="color: #444; line-height: 1.6;">
          Vous avez demandé la réinitialisation de votre mot de passe InkTracker.
        ...
        <p style="color: #aaa; font-size: 11px;">InkTracker — Suivi de tournois Lorcana</p>
      </div>
    `,
  });
}
